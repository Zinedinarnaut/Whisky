...
class ThumbnailProvider: QLThumbnailProvider {
    override func provideThumbnail(for request: QLFileThumbnailRequest,
        ...
        // Vector icon is aligned bottom-right
        let vectorIconFrame = CGRect(x: request.maximumSize.width - request.maximumSize.width * vectorIconScaleFactor,
                                     y: 0,
                                     width: request.maximumSize.width * vectorIconScaleFactor,
                                     height: request.maximumSize.height * vectorIconScaleFactor)
        do {
            var image: NSImage?
            ...
            let reply: QLThumbnailReply = QLThumbnailReply.init(contextSize: thumbnailSize) { () -> Bool in
                if let image = image {
                    ...
                    let vectorIcon = NSImage(named: NSImage.Name("Icon"))
                    vectorIcon?.draw(in: vectorIconFrame, from: .zero, operation: .sourceOver, fraction: 1)
                    return true
                }
