...
class Winetricks {
    ...
    static func runCommand(command: String, bottle: Bottle) async {
        ...
        let winetricksCmd = #"PATH=\"\#(VectorWineInstaller.binFolder.path):\#(resourcesURL.path(percentEncoded: false)):$PATH\" WINE=wine64 WINEPREFIX=\"\#(bottle.url.path)\" \"\#(winetricksURL.path(percentEncoded: false))\" \#(command)"#
