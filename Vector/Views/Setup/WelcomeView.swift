...
struct WelcomeView: View {
    ...
    @State var vectorWineInstalled: Bool?
        ...
        VStack {
            ...
            Form {
                InstallStatusView(isInstalled: $rosettaInstalled,
                                  ...
                                  name: "Rosetta")
                InstallStatusView(isInstalled: $vectorWineInstalled,
                                  shouldCheckInstallStatus: $shouldCheckInstallStatus,
                                  showUninstall: true,
                                  name: "VectorWine")
            }
            ...
            HStack {
                if let rosettaInstalled = rosettaInstalled,
                   let vectorWineInstalled = vectorWineInstalled {
                    if !rosettaInstalled || !vectorWineInstalled {
                        Button("setup.quit") {
                            exit(0)
                        }
                    ...
                    Spacer()
                    Button(rosettaInstalled && vectorWineInstalled ? "setup.done" : "setup.next") {
                        if !rosettaInstalled {
                            path.append(.rosetta)
                            return
                        }

                        if !vectorWineInstalled {
                            path.append(.vectorWineDownload)
                            return
                        }
    ...
    func checkInstallStatus() {
        ...
        vectorWineInstalled = VectorWineInstaller.isVectorWineInstalled()
    }
}
