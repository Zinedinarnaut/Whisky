    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandGroup(after: .importExport) {
                Button("open.logs") {
                    VectorApp.openLogsFolder()
                }
                .keyboardShortcut("L", modifiers: [.command])
                Button("kill.bottles") {
                    VectorApp.killBottles()
                }
                .keyboardShortcut("K", modifiers: [.command, .shift])
                Button("wine.clearShaderCaches") {
                    VectorApp.killBottles() // Better not make things more complicated for ourselves
                    VectorApp.wipeShaderCaches()
                }
            }
