...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            if checkVectorWineUpdates {
                let task = Task.detached {
                    return await VectorWineInstaller.shouldUpdateVectorWine()
                }
                let updateInfo = await task.value
                if updateInfo.0 {
                    let alert = NSAlert()
                    alert.messageText = String(localized: "update.vectorwine.title")
                    alert.informativeText = String(format: String(localized: "update.vectorwine.description"),
                                                   String(VectorWineInstaller.vectorWineVersion()
